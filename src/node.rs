...
impl NodeTrait for Node {
    fn process(&mut self, node_values: &ValuesById) -> f32 {
        match self {
            ...
            Node::AllPassNode(node) => node.process(node_values),
    ...
    fn get_input_ids(&self) -> Vec<usize> {
        match self {
            ...
            Node::AllPassNode(node) => node.get_input_ids(),
    ...
    fn update(&mut self, new_node: &Self) {
        match (self, new_node) {
            ...
            (Node::AllPassNode(node), Node::AllPassNode(new_node)) => node.update(new_node),
    ...
    fn get_is_pending(&self) -> bool {
        match self {
            ...
            Node::AllPassNode(node) => node.get_is_pending(),
...
impl HasId for Node {
    fn get_id(&self) -> usize {
        match self {
            ...
            Node::AllPassNode(node) => node.get_id(),
